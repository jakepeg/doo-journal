...
import type { WeeklyReminderSettings } from '../../../declarations/journal_backend/journal_backend.did';

interface WeeklyReminderSectionProps {
  settings?: WeeklyReminderSettings;
  onChange: (settings: WeeklyReminderSettings | undefined) => void;
}
...
export default function WeeklyReminderSection({ settings, onChange }: WeeklyReminderSectionProps) {
  ...
  const [dayOfWeek, setDayOfWeek] = useState(settings?.dayOfWeek ?? 1); // Default to Monday
  const [timeSlot, setTimeSlot] = useState<'morning' | 'afternoon'>(
    settings?.timeSlot ? 
      (Object.keys(settings.timeSlot)[0] as 'morning' | 'afternoon') : 
      'morning'
  );
  ...
  useEffect(() => {
    if (enabled) {
      const newSettings: WeeklyReminderSettings = {
        enabled,
        dayOfWeek,
        timeSlot: { [timeSlot]: null } as any // Motoko variant type
      };
  ...
  const handleEnableChange = async (newEnabled: boolean) => {
        ...
        {!hasPermission && enabled && (
          <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
            <p className="text-sm text-orange-800">
              <strong>Notification permission required:</strong> Please allow notifications to receive your weekly reminders.
            </p>
          </div>
        )}
      </CardContent>
